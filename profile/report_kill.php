...
if( isset($_POST['code']) and $_POST['code']!="" ){
	...
	$x = mysql_real_escape_string($_POST['map_x']);
	$y = mysql_real_escape_string($_POST['map_y']);
	$feed1 = mysql_real_escape_string($_POST['feed1']);
	$feed2 = mysql_real_escape_string($_POST['feed2']);
	
	$res = mysql_query("SELECT * FROM users WHERE player_code='$code' LIMIT 1");
	if(!mysql_num_rows($res)) 
		...
		if ($r['faction'] != 'HUMAN')
			...
			if( date("H") < "07" )
				...
				$starve = date("Y-m-d H:m:s", strtotime("+28 hours"));
			$change_human = mysql_query("UPDATE users SET faction='ZOMBIE', starve_time='$starve', kills=0 WHERE player_code='$code'") or die("Zombify Human Fail - UPDATE users SET (faction='ZOMBIE', starve_time='$starve', kills=0) WHERE player_code='$code'");
			$increment_kills=mysql_query("UPDATE users SET starve_time = DATE_ADD(NOW(), INTERVAL 2 DAY), kills=kills+1 WHERE gt_name='$gt_name'") or die("Count kills fail");
			$feed_zombies = mysql_query("UPDATE users SET starve_time = DATE_ADD(NOW(), INTERVAL 2 DAY) WHERE gt_name = '$feed1' or gt_name = '$feed2'") or die("Feed zombies fail");
		}
	}
