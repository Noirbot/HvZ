<?php
	...
	if($faction == 'admin'){
		$error=0;
		if( isset($_POST['id']) ){
			$id = (isset($_POST['id']) and $_POST['id']) ? mysql_real_escape_string($_POST['id']) : 'error';
			...
			if( $id=='error' or $name == 'error' or $start_datetime == 'error' or $end_datetime == 'error' or $release_datetime == 'error' or $location == 'error' or $faction == 'error' or $description == 'error')
				...
				mysql_query("UPDATE missions SET name='$name', start_datetime='$start_datetime', end_datetime='$end_datetime', release_datetime='$release_datetime', location='$location', faction='$faction', description='$description' WHERE id='$id'") or die(mysql_error());
